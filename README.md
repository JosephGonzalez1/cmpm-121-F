...
- Joseph is responsible for the technical core of the game by managing the integration and stability of React, Three.js, and Ammo.js. He establishes architectural patterns, reviews core system code, and guides the team on technical feasibility, optimization strategies, and feature implementation.
...
1. **Engine**:
- We're using **React + TypeScript** with **Three.js** and **Ammo.js** as our core engine layer. Three.js provides WebGL rendering for 3D environments, React offers component-based architecture for UI and state management, and Ammo.js handles physics simulation and collision detection.
2. **Language**:
- TypeScript/JSON -> We will be using TypeScript as our main programming languages because our team is already familiar with it and it integrates directly with React and Three.js. We will also be using JSON for structuring external data, storing level information, managing configuration files, and organizing reusable game elements.
3. **Tools**: 
- Our project uses a focused set of tools to support art creation, rapid prototyping, clean coding practices, and efficient testing.
    ...
    - **Development Environment - Deno and Vite**: We're using Deno as our secure JavaScript/TypeScript runtime with built-in dependency management. Vite serves as our fast build tool and development server, providing hot module replacement and optimized builds.
...
- **Connecting art and code smoothly**: React, Three.js, and Ammo.js each solve different problems, but combining them introduces complexity. It allows physics updates from Ammo.js with the rendering loop in Three.js while keeping the React state predictable.
...
- By using Three.js and Ammo.js, it helps us understand how real-time 3D rendering, physics simulation, and game loops work at a lower level, instead of relying on Unity or Unreal. As far as React, we hope to learn how to structure game interfaces, configuration panels, and gameplay management using a component-based design.

...
const calculateThrowVelocity = (
  camera: THREE.Camera,
  throwPower: number
): { velocity: CANNON.Vec3; angular: CANNON.Vec3 } => {
  const direction = new THREE.Vector3(0, 0, -1);
  direction.applyQuaternion(camera.quaternion);
  direction.y += 0.4;
  direction.normalize();

  const power = (throwPower / 100) * BASKETBALL_CONFIG.BALL.THROW.POWER_MULTIPLIER 
    + BASKETBALL_CONFIG.BALL.THROW.POWER_OFFSET;
...
const checkBallThroughHoop = (
  ballPos: CANNON.Vec3,
  hoopCenter: { x: number; y: number; z: number },
  lastY: number
): boolean => {
  ...
  return horizontalDist < BASKETBALL_CONFIG.GAME.SCORE_CHECK_DISTANCE 
    && lastY > hoopCenter.y 
    && currentY < hoopCenter.y;
};
...
const CourtLines = React.memo(({ 
  ...
  <>
    {/* Pickup Circle */}
    <mesh position={[position[0], position[1] + 0.01, position[2] + 7]} rotation={[-Math.PI / 2, 0, 0]}>
...
export function BasketballGame({ position, zoneSize }: BasketballGameProps) {
  ...
  const throwBall = useCallback(() => {
    ...
    const throwData = calculateThrowVelocity(camera, throwPower);

    // Apply velocity based on charge power
    ballBodyRef.current.velocity.copy(throwData.velocity);
    ballBodyRef.current.angularVelocity.copy(throwData.angular);
  ...
  const checkScore = useCallback(() => {
    ...
    if (checkBallThroughHoop(ballPos, hoopCenter, lastY)) {
      incrementBasketballScore();
  ...
  const handleProximityCheck = useCallback(() => {
    ...
    const isNear = distance < BASKETBALL_CONFIG.GAME.PROXIMITY_RADIUS;
  ...
  }, [camera, position, isNearBasketball, isBasketballActive, isLocked, keys, resetInteract, setIsNearBasketball, exitGame, enterBasketballZone]);

  // THROW CHARGING LOGIC
  // --------------------------------------------------------------------------
  const handleThrowCharging = useCallback((delta: number) => {
    if (isChargingThrow) {
      const newPower = Math.min(100, throwPower + delta * 80);
  ...
  // BALL HOLDING LOGIC
  // --------------------------------------------------------------------------
  const updateBallHolding = useCallback(() => {
  ...
  const handleBallReset = useCallback((delta: number) => {
    if (!isHoldingBall && hasThrownBallRef.current && ballBodyRef.current) {
      ...
      if (isOnGround) {
        ...
        if (ballGroundedTimeRef.current > BASKETBALL_CONFIG.GAME.GROUNDED_CHECK_TIME) {
  ...
  useEffect(() => {
    const handleMouseDown = (e: MouseEvent) => {
      ...
      if (!isHoldingBall && ballBodyRef.current && ballMeshRef.current) {
        ...
        if (distance < BASKETBALL_CONFIG.GAME.MAX_PICKUP_DISTANCE) {
  ...
  useEffect(() => {
    ...
    const northWall = new CANNON.Body({
      mass: 0,
      position: new CANNON.Vec3(position[0], position[1] + BASKETBALL_CONFIG.GAME.WALL_HEIGHT / 2, position[2] - zoneSize[2] / 2),
      shape: new CANNON.Box(new CANNON.Vec3(zoneSize[0] / 2, BASKETBALL_CONFIG.GAME.WALL_HEIGHT / 2, BASKETBALL_CONFIG.GAME.WALL_THICKNESS)),
    ...
    const eastWall = new CANNON.Body({
      mass: 0,
      position: new CANNON.Vec3(position[0] + zoneSize[0] / 2, position[1] + BASKETBALL_CONFIG.GAME.WALL_HEIGHT / 2, position[2]),
      shape: new CANNON.Box(new CANNON.Vec3(BASKETBALL_CONFIG.GAME.WALL_THICKNESS, BASKETBALL_CONFIG.GAME.WALL_HEIGHT / 2, zoneSize[2] / 2)),
      material: wallMaterial,
    });
    ...
    const ceiling = new CANNON.Body({
      mass: 0,
      position: new CANNON.Vec3(position[0], position[1] + BASKETBALL_CONFIG.GAME.WALL_HEIGHT, position[2]),
      shape: new CANNON.Box(new CANNON.Vec3(zoneSize[0] / 2, 0.1, zoneSize[2] / 2)),
      material: wallMaterial,
    });
  ...
  return (
    <group>
      {/* Court Boundaries */}
      <CourtBoundaries position={position} zoneSize={zoneSize} />
      ...
      {/* Court Lines */}
      <CourtLines hoopPosition={hoopPosition} position={position} />
    </group>
  );
}